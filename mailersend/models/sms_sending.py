...
class SmsSendRequest(MailerSendBaseModel):
    ...
    def validate_to_numbers(cls, v):
        ...
        for number in v:
            if not number.startswith('+'):
                raise ValueError("All phone numbers must be in E164 format (start with +)")
        return v
    ...
    def validate_personalization(cls, v):
        ...
        if v.personalization:
            ...
            if invalid_numbers:
                raise ValueError(f"Personalization phone numbers not in recipient list: {invalid_numbers}")
