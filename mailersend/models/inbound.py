...
class InboundFilter(BaseModel):
    ...
    key: Optional[str] = Field(None, description="Filter key (required for match_header)")
    ...
    def validate_type(cls, v):
        if v is None:
            return v
        valid_types = ['catch_all', 'catch_recipient', 'match_all', 'match_sender', 'match_domain', 'match_header']
...
class InboundRoute(BaseModel):
    ...
    filters: List[InboundFilter] = Field(default_factory=list, description="List of filters")
    forwards: List[InboundForward] = Field(default_factory=list, description="List of forwards")
    priority: Optional[int] = Field(None, description="Route priority")
    mxValues: Optional[InboundMxValues] = Field(None, description="MX record values", alias="mxValues")
...
class InboundCreateRequest(BaseModel):
    ...
    inbound_priority: Optional[int] = Field(None, ge=0, le=100, description="Inbound priority")
    catch_filter: List[InboundFilterGroup] = Field(..., description="Catch filter configuration")
    catch_type: Optional[str] = Field(None, description="Catch type (all or one)")
    match_filter: List[InboundFilterGroup] = Field(..., description="Match filter configuration")
...
class InboundListResponse(BaseModel):
    ...
    data: List[InboundRoute] = Field(default_factory=list, description="List of inbound routes")
