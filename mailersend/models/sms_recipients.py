...
class SmsRecipientsListQueryParams(BaseModel):
    ...
    status: Optional[SmsRecipientStatus] = Field(None, description="Recipient status filter")
...
class SmsRecipientsListRequest(BaseModel):
    ...
    query_params: SmsRecipientsListQueryParams = Field(default_factory=SmsRecipientsListQueryParams)
...
class SmsRecipientDetails(SmsRecipient):
    ...
    sms: List[SmsMessage] = Field(default_factory=list, description="SMS messages sent to this recipient")
