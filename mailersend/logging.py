...
class RequestLogger:
    ...
    def start_request(self, method: str, url: str, params: Optional[Dict] = None, 
        ...
        self.logger.info(f"🚀 Starting {method} request to {url}", 
                        extra={'request_id': self.request_id})
        
        if params:
            sanitized_params = SensitiveDataFormatter()._sanitize_dict(params)
            self.logger.debug(f"📋 Query params: {json.dumps(sanitized_params, indent=2)}", 
                            extra={'request_id': self.request_id})
        
        if body:
            sanitized_body = SensitiveDataFormatter()._sanitize_dict(body)
            self.logger.debug(f"📦 Request body: {json.dumps(sanitized_body, indent=2)}", 
                            extra={'request_id': self.request_id})
    ...
    def log_response(self, response, duration: Optional[float] = None):
        ...
        self.logger.info(f"{status_emoji} Response {response.status_code}{duration_str}", 
                        extra={'request_id': self.request_id})
        ...
        if filtered_headers:
            self.logger.debug(f"📄 Response headers: {json.dumps(filtered_headers, indent=2)}", 
                            extra={'request_id': self.request_id})
    ...
    def log_retry(self, attempt: int, delay: float):
        """Log retry attempts."""
        self.logger.warning(f"🔄 Retrying request (attempt {attempt}) after {delay}s delay", 
                           extra={'request_id': self.request_id})