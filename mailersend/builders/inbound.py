...
class InboundBuilder:
    ...
    def add_catch_filter(self, filter_type: str, filters: Optional[List[Dict[str, Any]]] = None) -> 'InboundBuilder':
    ...
    def add_match_filter(self, filter_type: str, filters: Optional[List[Dict[str, Any]]] = None) -> 'InboundBuilder':
    ...
    def add_forward(self, forward_type: str, value: str, forward_id: Optional[str] = None, secret: Optional[str] = None) -> 'InboundBuilder':
        """Add a forward configuration."""
        forward = InboundForward(id=forward_id, type=forward_type, value=value, secret=secret)
    ...
    def add_webhook_forward(self, url: str, secret: Optional[str] = None) -> 'InboundBuilder':
    ...
    def catch_recipient(self, filters: List[Dict[str, Any]], catch_type: str = 'all') -> 'InboundBuilder':
    ...
    def match_sender(self, filters: List[Dict[str, Any]], match_type: str = 'all') -> 'InboundBuilder':
        """Add a match sender filter with conditions."""
        self.add_match_filter('match_sender', filters)
        self._match_type = match_type
        return self

    def match_domain(self, filters: List[Dict[str, Any]], match_type: str = 'all') -> 'InboundBuilder':
        """Add a match domain filter with conditions."""
        self.add_match_filter('match_domain', filters)
        self._match_type = match_type
        return self

    def match_header(self, filters: List[Dict[str, Any]], match_type: str = 'all') -> 'InboundBuilder':
    ...
    def build_create_request(self) -> InboundCreateRequest:
        ...
        if self._domain_enabled is None:
            raise MailerSendValidationError("Domain enabled flag is required for create request")
        if not self._catch_filter:
            raise MailerSendValidationError("At least one catch filter is required for create request")
        if not self._match_filter:
            raise MailerSendValidationError("At least one match filter is required for create request")
        if not self._forwards:
            raise MailerSendValidationError("At least one forward is required for create request")
    ...
    def build_update_request(self) -> InboundUpdateRequest:
        ...
        if self._domain_enabled is None:
            raise MailerSendValidationError("Domain enabled flag is required for update request")
        if not self._catch_filter:
            raise MailerSendValidationError("At least one catch filter is required for update request")
        if not self._match_filter:
            raise MailerSendValidationError("At least one match filter is required for update request")
        if not self._forwards:
            raise MailerSendValidationError("At least one forward is required for update request")
