...
class SmsInbounds(BaseResource):
    ...
    def get_sms_inbound(self, request: SmsInboundGetRequest) -> APIResponse:
        ...
        response = self.client.request(method="GET", path=f"sms-inbounds/{request.sms_inbound_id}")
        return self._create_response(response)
    ...
    def create_sms_inbound(self, request: SmsInboundCreateRequest) -> APIResponse:
        ...
        """
        self.logger.info(f"Creating SMS inbound: {request.name} for SMS number: {request.sms_number_id}")
        response = self.client.request(method="POST", path="sms-inbounds", data=request.to_request_body())
        return self._create_response(response)
    ...
    def delete_sms_inbound(self, request: SmsInboundDeleteRequest) -> APIResponse:
        ...
        response = self.client.request(method="DELETE", path=f"sms-inbounds/{request.sms_inbound_id}")
        return self._create_response(response) 