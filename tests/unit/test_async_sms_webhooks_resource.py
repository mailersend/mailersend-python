"""Tests for AsyncSmsWebhooks resource."""

from unittest.mock import AsyncMock, MagicMock

from mailersend.resources.sms_webhooks import AsyncSmsWebhooks
...
class TestAsyncSmsWebhooks:
    def setup_method(self):
        self.mock_client = _make_mock_client()
        self.resource = AsyncSmsWebhooks(self.mock_client)
    ...
    async def test_list_sms_webhooks_returns_api_response(self):
        ...
        result = await self.resource.list_sms_webhooks(request)
    ...
    async def test_create_sms_webhook_returns_api_response(self):
        ...
        result = await self.resource.create_sms_webhook(request)
        assert isinstance(result, APIResponse)
    ...
    async def test_create_sms_webhook_calls_correct_endpoint(self):
        ...
        await self.resource.create_sms_webhook(request)
        call = self.mock_client.request.call_args
