...
def test_dict_like_interface_for_json():
    ...
    response = APIResponse(data=data, headers=headers, status_code=201)
...
def test_json_serialization_with_complex_data():
    ...
    complex_data = {
        ...
        "meta": {"total": 2, "processed": 2},
    }

    headers = {"x-request-id": "req-789", "x-total-count": "2"}

    response = APIResponse(data=complex_data, headers=headers, status_code=200)
