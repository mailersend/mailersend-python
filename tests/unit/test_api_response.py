...
class TestAPIResponse:
    ...
    def test_success_property(self):
        ...
        for status_code in [200, 201, 204, 299]:
            response = APIResponse(data={}, headers={}, status_code=status_code)
            assert response.success is True
    ...
    def test_retry_after_property(self):
        ...
        response = APIResponse(data={}, headers={"Retry-After": "60"}, status_code=429)
        assert response.retry_after == 60

        # Test with lowercase header
        response = APIResponse(data={}, headers={"retry-after": "120"}, status_code=429)
        assert response.retry_after == 120

        # Test without header
        response = APIResponse(data={}, headers={}, status_code=200)
        assert response.retry_after is None

        # Test with invalid value
        response = APIResponse(
            data={}, headers={"Retry-After": "invalid"}, status_code=429
        )
        assert response.retry_after is None
    ...
    def test_complex_data_structures(self):
        ...
        assert (
            response.data_items[0]["name"] == "Item 1"
        )  # Use data_ prefix for conflicts
