...
def sms_send_request_with_personalization(sms_phone_number):
    ...
    return SmsSendRequest(
        ...
        personalization=[
            SmsPersonalization(phone_number=recipient_number, data={"name": "John"})
        ],
    )
...
class TestSmsSendingIntegration:
    ...
    def test_send_sms_basic(self, email_client, basic_sms_send_request):
        ...
        assert (
            ...
            or "from" in error_str
        )

    @vcr.use_cassette("sms_send_with_personalization.yaml")
    def test_send_sms_with_personalization(
        self, email_client, sms_send_request_with_personalization
    ):
        ...
        # This will likely fail due to invalid phone numbers or missing SMS configuration
        with pytest.raises((BadRequestError, ResourceNotFoundError)) as exc_info:
            response = email_client.sms_sending.send(
                sms_send_request_with_personalization
            )
    ...
    def test_send_sms_long_message(self, email_client, sms_phone_number):
        ...
        request = SmsSendRequest(
            from_number=sms_phone_number, to=[sms_phone_number], text=long_text
        )
    ...
    def test_send_sms_validation_error(self, email_client):
        ...
        error_str = str(exc_info.value).lower()
        assert "attribute" in error_str or "to_json" in error_str
    ...
    def test_sms_send_model_validation_text_length(self):
        ...
        # Test empty text
        with pytest.raises(ValueError) as exc_info:
            SmsSendRequest(from_number="+1234567890", to=["+1234567891"], text="")
    ...
    def test_sms_send_to_json(self, sms_phone_number):
        ...
        request = SmsSendRequest(
            ...
            text="Hello {{name}}!",
            personalization=[
                SmsPersonalization(phone_number=recipient1, data={"name": "John"}),
                SmsPersonalization(phone_number=recipient2, data={"name": "Jane"}),
            ],
        )
    ...
    def test_sms_send_to_json_no_personalization(self, sms_phone_number):
        ...
        request = SmsSendRequest(
            from_number=sms_phone_number, to=[recipient], text="Simple message"
        )
    ...
    def test_phone_number_edge_cases(self):
        ...
        # Valid E164 numbers
        valid_numbers = ["+1234567890", "+44123456789", "+33123456789", "+49123456789"]

        for number in valid_numbers:
            # Should not raise exception
            request = SmsSendRequest(from_number=number, to=[number], text="Test")
    ...
    def test_text_message_edge_cases(self):
        ...
        max_text = "x" * 2048
        request = SmsSendRequest(
            from_number="+1234567890", to=["+1234567891"], text=max_text
        )
    ...
    def test_recipients_limit_validation(self):
        ...
        request = SmsSendRequest(
            from_number="+1234567890", to=fifty_recipients, text="Bulk message"
        )
        assert len(request.to) == 50

        # Test single recipient
        request2 = SmsSendRequest(
            from_number="+1234567890", to=["+1234567891"], text="Single message"
        )
