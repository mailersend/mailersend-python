...
def basic_tokens_list_request():
    ...
    return TokensListRequest(query_params=TokensListQueryParams(page=1, limit=10))
...
class TestTokensIntegration:
    ...
    def test_update_token_not_found(self, email_client):
        ...
        request = TokenUpdateRequest(token_id="test-token-id", status="pause")
    ...
    def test_update_token_name_not_found(self, email_client):
        ...
        request = TokenUpdateNameRequest(
            token_id="test-token-id", name="Updated Token Name"
        )
    ...
    def test_delete_token_not_found(self, email_client, token_get_request):
        ...
        delete_request = TokenDeleteRequest(token_id=token_get_request.token_id)
    ...
    def test_token_create_model_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            TokenCreateRequest(name="", domain_id="test-domain", scopes=["email_full"])
        ...
        # Test empty scopes
        with pytest.raises(ValidationError) as exc_info:
            TokenCreateRequest(name="Test Token", domain_id="test-domain", scopes=[])
        assert "list should have at least 1 item" in str(exc_info.value).lower()

        # Test invalid scopes
        with pytest.raises(ValueError) as exc_info:
            TokenCreateRequest(
                name="Test Token", domain_id="test-domain", scopes=["invalid_scope"]
            )
    ...
    def test_token_update_name_model_validation(self):
        ...
        # Test empty name
        with pytest.raises(ValueError) as exc_info:
            TokenUpdateNameRequest(token_id="test-id", name="")
        assert "token name cannot be empty" in str(exc_info.value).lower()
...
class TestTokensBuilderIntegration:
    ...
    def test_builder_update_not_found(self, email_client):
        ...
        builder = TokensBuilder()
        request = builder.token_id("test-token-id").pause().build_token_update()
    ...
    def test_builder_update_name_not_found(self, email_client):
        ...
        builder = TokensBuilder()
        request = (
            builder.token_id("test-token-id")
            .name("Updated Name")
    ...
    def test_comprehensive_tokens_workflow(self, email_client, test_domain_id):
        ...
        # Test create token
        create_request = TokenCreateRequest(
            name="Test Token", domain_id=test_domain_id, scopes=["email_full"]
        )
        try:
        ...
        builder_get_request = builder.token_id(
